...
This document describes how to deploy `Kisser` instances via _Chronicle Protocol_'s [`Greenhouse`](https://github.com/chronicleprotocol/greenhouse) contract factory.
...
- `SALT`: The salt to deploy the `Kisser` instance to
...
- `INITIAL_AUTHED`: The address being auth'ed on the newly deployed `Kisser` instance
...
1. Adjust the `Kisser_COUNTER`'s name in `src/Kisser.sol` and remove the `TODO` comment
2. Adjust the import of the `Chronicle_COUNTER` in `script/Kisser.s.sol` and remove the `TODO` comment
